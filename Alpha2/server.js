...
app.post('/process_register', function (request, response) {
    ...
    if (typeof user_data[username_entered] == 'undefined') {
        if (password_entered == password_repeat) {
        ...
        response.redirect('invoice.html?' + qs.stringify(request.query));
            ...
            response.send(`Passwords do not match, please try again <br> <input type="button" value="Go back!" onclick="history.back()">`);
        }
    } else {
        //Need to prove this is undefined so the username is not exit on the user_data.json
        response.send(`${username_entered} has been used, please try again <br> <input type="button" value="Go back!" onclick="history.back()">`);
    }
...
app.post('/process_login', function (request, response, next) {
    ...
    if (typeof user_data[username_entered] != 'undefined') {
        ...
        if (user_data[username_entered]['password'] == password_entered) {
            ...
            response.send(`${username_entered} password wrong <br> <input type="button" value="Go back!" onclick="history.back()">`);
        ...
        //refer to the line37
        response.send(`${username_entered} not found <br> <input type="button" value="Go back!" onclick="history.back()">`);
    }
});
